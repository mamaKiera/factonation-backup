    ...
    return(
        <div className="flex justify-center my-10">
            ...
            {courses.map((course, i) => {
                return (
            <div key={i} className="flex gap-5 rounded-xl w-[920px] h-[340px] bg-neutral-100 px-6 items-center drop-shadow-md ">
                <div>
                <Image src={course.image} alt='Industry image' className="flex items-center rounded-xl h-[250px] w-[480px]"/>   
                </div>
                <div className="flex flex-col w-[710px] h-[320px] p-2 gap-3 justify-center">
                    <div className="flex flex-col gap-2">
                       <p className="text-xs text-primary-button">{course.module}</p>
                       <p className="text-xl font-semibold">{course.name}</p>
                       <p className="text-sm text-stone-600">{course.detail}</p>
                       <p className="text-xs font-semibold">จำนวนวิดีโอ {course.video} วิดีโอ</p>
                    </div>
                    <div className="flex gap-10 justify-between border-t border-primary-button py-1 ">
                        <p className="text-xs font-semibold my-2  bg-neutral-100">{course.price}</p>
                        <div className="flex items-center gap-0">
                        <Button className="text-xs font-semibold bg-neutral-100">ดูรายละเอียดคอร์์ส</Button>
                        <ArrowRight size={10} />
                        </div>
                    </div>
                </div>
            ...
            {courses.map((course, i) => {
                return (
            <div key={i} className="flex flex-col gap-3 rounded-xl w-[300px] h-[450px] bg-neutral-100  ">
               <div>
               <Image src={course.image} alt='Industry image' className="flex items-center rounded-t-xl w-full"/>           
               </div>
